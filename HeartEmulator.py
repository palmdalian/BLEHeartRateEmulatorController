...
#Don't forget to sudo chmod a+rw /dev/ttyAMA0
subprocess.check_call(["chmod", "a+rw", "/dev/ttyAMA0"])
...
def sendKey(key, hr):
    print "New heart rate target: " + str(hr)
    ser.write(key)

def playBeeps(zoneNumber):
    if zoneNumber == 0: #slowed down sound
        pygame.mixer.music.load("/home/pi/sounds/slow.mp3")
        pygame.mixer.music.play()
    elif zoneNumber == 1:
        pygame.mixer.music.load("/home/pi/sounds/beeps1.mp3")
        pygame.mixer.music.play()
    elif zoneNumber == 2:
        pygame.mixer.music.load("/home/pi/sounds/beeps2.mp3")
        pygame.mixer.music.play()
    elif zoneNumber == 3:
        pygame.mixer.music.load("/home/pi/sounds/beeps3.mp3")
        pygame.mixer.music.play()
    elif zoneNumber == 4:
        pygame.mixer.music.load("/home/pi/sounds/beeps4.mp3")
        pygame.mixer.music.play()
    elif zoneNumber == 5:
        pygame.mixer.music.load("/home/pi/sounds/beeps5.mp3")
        pygame.mixer.music.play()

    elif zoneNumber == 6:#You fail sound
        pygame.mixer.music.load("/home/pi/sounds/fail.mp3")
        pygame.mixer.music.play()
    elif zoneNumber == 7:#You're safe sound
        pygame.mixer.music.load("/home/pi/sounds/ok.mp3")
        pygame.mixer.music.play()
...
def main(addr=None, sqlfile=None, gatttool="gatttool", check_battery=False):
    ...
    while retry:
        ...
        while 1:
            ...
            gt = pexpect.spawn(gatttool + " -t random -b " + addr + " --interactive")
            gt.expect(r"\[LE\]>")
            gt.sendline("connect")
        ...
        while 1:
            ...
            if firstRun == 0:
                currentHeart = res["hr"]
                print "Target heart rate starts at " + str(currentHeart)
                firstRun = 1



            if res["hr"] > (currentHeart + 5):
                print "Danger Zone in " + str(3 - dangerZone)
                if dangerZone >= 3:
                    #Crossed the threshold, speed up
                    sendKey('1', res["hr"])
                    dangerZone = 0
                    currentHeart = res["hr"]
                    playBeeps(7) #You fail sound
                else:
                    dangerZone += 1
                    playBeeps(dangerZone)
                    if safeZone > 0:
                        print "Resetting safe zone"
                        safeZone = 0
            else:
                if dangerZone>0:
                    playBeeps(dangerZone)
            ...
            if res["hr"] <= (currentHeart - 5):
                print "Safe Zone in " + str(3-safeZone)
                if safeZone >= 3:
                    sendKey('0', res["hr"])
                    dangerZone = 0
                    currentHeart = res["hr"]
                    playBeeps(0) #You slowed sound
                else:
                    safeZone += 1
                    if dangerZone > 0:
                        print "Resetting danger zone"
                        dangerZone = 0
                        playBeeps(6) #You're safe sound

            
            if sqlfile is None:
                # log.info("Heart rate: " + str(res["hr"]))
		# start_new_thread(setLEDStrength, (res["hr"],))
                continue
